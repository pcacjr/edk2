...
/**
  ...
  @param[out] LastAddressableBlock  Last addressable block
...
FindAnchorVolumeDescriptorPointer (
  ...
  OUT  EFI_LBA                               *LastAddressableBlock
  )
{
  EFI_STATUS                            Status;
  UINT32                                BlockSize;
  ...
  *LastAddressableBlock = EndLBA;
  ...
  if (DescriptorTag->TagIdentifier == UdfAnchorVolumeDescriptorPointer) {
    DEBUG ((EFI_D_INFO, "%a: found AVDP at block %d\n", __FUNCTION__, 256));
    AvdpsCount++;
  }
  ...
  if (DescriptorTag->TagIdentifier == UdfAnchorVolumeDescriptorPointer &&
      ++AvdpsCount == 2) {
    DEBUG ((EFI_D_INFO, "%a: found AVDP at block %Ld\n", __FUNCTION__,
            EndLBA - 256));
    return EFI_SUCCESS;
  }
  ...
  //
  // Search backwards for an AVDP from block N-1 through N-512. If any AVDP is
  // found, then correct last block number for the new UDF partition child
  // handle.
  //
  Size = 512u * BlockSize;
  ...
  //
  // Read 512 consecutive disk blocks
  //
  Status = DiskIo->ReadDisk (
    DiskIo,
    BlockIo->Media->MediaId,
    MultU64x32 ((UINT64)EndLBA - 512, BlockSize),
    Size,
    AnchorPoints
    );
  if (EFI_ERROR (Status)) {
  ...
  //
  // Search for AVDP from N-1 through N-512
  //
  for (AnchorPointPtr = (VOID *)((UINTN)AnchorPoints + 510 * BlockSize);
       AnchorPointPtr >= AnchorPoints;
       AnchorPointPtr--) {
    DescriptorTag = &AnchorPointPtr->DescriptorTag;

    if (DescriptorTag->TagIdentifier == UdfAnchorVolumeDescriptorPointer) {
      LastAvdpBlockNum =
        EndLBA - (512 - DivU64x32 ((UINT64)((UINTN)AnchorPointPtr -
                                            (UINTN)AnchorPoints),
                                   BlockSize));
      DEBUG ((EFI_D_INFO, "%a: found AVDP at block %Ld\n", __FUNCTION__,
              LastAvdpBlockNum));
      DEBUG ((EFI_D_INFO, "%a: correcting last block from %Ld to %Ld\n",
              __FUNCTION__, EndLBA, LastAvdpBlockNum));
      //
      // Save AVDP from last block
      //
      CopyMem (AnchorPoint, AnchorPointPtr, sizeof (*AnchorPointPtr));
      //
      // Set last block number to the block where AVDP was found
      //
      *LastAddressableBlock = LastAvdpBlockNum;
      Status = EFI_SUCCESS;
      break;
    }
  }
...
/**
  ...
  @param[in]  LastAddressableBlock  Last addressable block in disk.
...
FindLogicalVolumeLocation (
  ...
  IN   EFI_LBA                               LastAddressableBlock,
  ...
  )
{
  ...
  if (SeqBlocksNum < 16 || (EFI_LBA)SeqBlocksNum > LastAddressableBlock + 1) {
    return EFI_VOLUME_CORRUPTED;
  }
  ...
  if (SeqStartBlock > LastAddressableBlock ||
      SeqStartBlock + SeqBlocksNum - 1 > LastAddressableBlock) {
    return EFI_VOLUME_CORRUPTED;
  }
  ...
  if (!EFI_ERROR (Status) && LvdsCount == 1) {
    *MainVdsStartBlock = GuardMainVdsStartBlock;
    ...
    // LastAddressableBlock.
    //
    *MainVdsEndBlock = LastAddressableBlock;
